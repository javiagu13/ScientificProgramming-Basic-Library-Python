...
def datasetDiscretizeEF(data, n_bins):
  for i in range(0,len(data[1,])):
    discrete = atributeDiscretizeEF(data[:,i],n_bins)
    print(discrete)
    col=discrete.copy()
    data[:,i]=col
  return data
...
def datasetDiscretizeEW(data, n_bins):
  for i in range(0,len(data[1,])):
    discrete, cutoff = atributeDiscretizeEW(data[:,i],n_bins)
    data[:,i]=discrete
  return data
...
def datasetNormalization(data):
  arr=[]
  for i in range(0,len(data[1,])):
    data[:,i] = variableNormalization(data[:,i])
    print(data)
  return data
...
def datasetEstandarization(data):
  arr=[]
  for i in range(0,len(data[1,])):
    data[:,i] = variableEstandarization(data[:,i])
    print(data)
  return data
